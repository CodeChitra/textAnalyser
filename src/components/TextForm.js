...
export default function TextForm(props) {
  ...
  //Setting the Text state variable value that is what written in textarea element
  ...
  return (
    <>
      <div>
        ...
        <button disabled={Text.length === 0} className='btn btn-primary mx-1 my-1' onClick={HandleSpaceClick}>Delete Extra Spaces</button>
      </div>
      <div className="container my-2">
        <h2>Your Text Summary</h2>
        <p id='counter'>Words are {Text.split(/\s+/).filter((element) => { return element.length !== 0 }).length} and Characters are {Text.length} </p>
        <p>You can read it in {Text.split(/\s+/).filter((element) => { return element.length !== 0 }).length * 0.008} minutes</p>
